...
function Home() {
  ...
  const handleLightAndDarkMode = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/v1/user/update-theme`,
  ...
  return (
    <div
      ...
      } h-screen md:flex-row overflow-scroll`}
    >
      <div
        className={`flex fixed items-center top-0 z-50 right-0 w-full px-3 pb-2 pt-3 justify-between md:hidden ${
          isDark ? "bg-[#161616]" : "bg-white"
        }`}
      >
        ...
        <div className="flex relative justify-center gap-5 items-center">
          <HiMiniPaperAirplane
            ...
            className={`cursor-pointer rotate-[-50deg] ${
          ...
          {isDark ? (
            <MdLightMode
              ...
              className={`${
                isDark ? "text-white" : "text-black"
              } w-5 h-5 cursor-pointer relative top-[2px]`}
            />
          ) : (
            <HiMoon
              onClick={handleLightAndDarkMode}
              className={`${
                isDark ? "text-white" : "text-black"
              } relative top-[1px] cursor-pointer w-5 h-5`}
            />
          )}
          {unreadUsers?.length ? (
            <>
              {" "}
              <span className="bg-red-500 h-2 w-2 absolute left-[-3px] bottom-[-5px] rounded-full"></span>{" "}
            </>
          ) : (
            ""
          )}
          {unreadUsers?.length ? (
            <span
              ...
              } absolute left-3 z-10 bottom-3 w-4 h-4 flex text-[12px] font-semibold items-center justify-center p-1 rounded-full`}
            >
              {unreadUsers?.length}
            </span>
          ) : (
      ...
      <div className="flex-1 md:mt-0 mt-[70px] px-3 w-full flex flex-col justify-start border-r border-gray-300">
        <div className="flex items-center md:hidden justify-start py-3  md:px-2 overflow-x-scroll">
